...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["editGambar"]) && $_FILES["editGambar"]["error"] == 0) {
        ...
        $path_gambar = "uploads/konten_galeri/" . basename($editGambar["name"]);

        // Update data termasuk gambar baru
        $sql = "UPDATE galeri SET path_gambar = '$path_gambar', judul = '$judul', deskripsi = '$deskripsi' WHERE id_galeri = $id";
        if ($conn->query($sql) === TRUE) {
            ...
            echo "<script>
                    Swal.fire({
                        ...
                        window.location.href = '?q=pgaleri';
                    });
                  </script>";
            exit();
        } else {
